...
int main(void)
{
   ...
   for (;;)
   {
      ...
      int id = ntohs(packetMessage->id); /*RequestId sent from the Client*/
      int count = ntohs(packetMessage->Count); /*The count number sent from the client*/
      ...
      bytes_sent = sendto(sock_server, &packets, sizeof(packets), 0,
                          (struct sockaddr *)&client_addr, client_addr_len);
      ...
      printf("The Count of the request packet received is : %d\n", count);
      printf("The total number of response packets transmitted is : %d\n", totalNumberPacketsTransmitted);
      printf("The total number of bytes transmitted is : %d\n", totalNUmberBytesTransmitted);
      printf("The total Sequence Number is : %d\n", sumSequenceNumber);
      printf("The total checksum is : %d\n", checksum);
      printf("-----------------------------------\n");
      totalNumberPacketsTransmitted=0;
      totalNUmberBytesTransmitted=0;
      sumSequenceNumber=0;
      checksum=0;
